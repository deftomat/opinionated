/**
 * Modified copy of "early-exit" rule from "tslint-consistent-codestyle".
 */
/* tslint:disable */
import * as Lint from 'tslint';
import { isBlock, isCaseOrDefaultClause, isFunctionScopeBoundary, isIfStatement } from 'tsutils';
...
function walk(ctx: Lint.WalkContext<IOptions>) {
  ...
  return ts.forEachChild(sourceFile, function cb(node): void {
    if (
      isIfStatement(node) &&
      (!ignoreConstructor || !isConstructorClosestFunctionScopeBoundary(node))
    )
      check(node);
  ...
  function check(node: ts.IfStatement): void {
    ...
    if (elseStatement === undefined) {
      if (isLarge(thenSize)) fail(failureString(exit));
      return;
    }

    // Never fail if there's an `else if`.
    if (elseStatement.kind === ts.SyntaxKind.IfStatement) return;

    if (maxLineLength === 0) return fail(failureStringAlways(exit));

    if (!hasReturnStatement(thenStatement) && hasReturnStatement(elseStatement)) {
      fail(
        "Only the 'else' branch contains return statement. Prefer to invert the condition and 'return' early."
      );
    }
    ...
    // }
    function hasReturnStatement(statement: ts.Statement) {
      if (!isBlock(statement)) return false;
      return statement.statements.some(st => st.kind === ts.SyntaxKind.ReturnStatement);
    }
...
function size(node: ts.Node, sourceFile: ts.SourceFile): number {
  return isBlock(node)
    ? node.statements.length === 0
      ? 0
      : diff(node.statements[0].getStart(sourceFile), node.statements.end, sourceFile)
    : diff(node.getStart(sourceFile), node.end, sourceFile);
}

function diff(start: number, end: number, sourceFile: ts.SourceFile): number {
  return (
    ts.getLineAndCharacterOfPosition(sourceFile, end).line -
...
function isLastStatement(
  ...
  i: number = statements.length - 1
): boolean {
  while (true) {
    const statement = statements[i];
    if (statement === ifStatement) return true;
    if (statement.kind !== ts.SyntaxKind.FunctionDeclaration) return false;
    if (i === 0)
